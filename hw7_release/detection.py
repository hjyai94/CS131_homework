...
def hog_feature(image, pixel_per_cell = 8):
    ...
    '''
    ### YOUR CODE HERE
    hogFeature, hogImage = feature.hog(image, orientations=8, pixels_per_cell=(pixel_per_cell, pixel_per_cell),
...
def pyramid(image, scale=0.9, minSize=(200, 100)):
    ...
    while image.shape[0] > minSize[0] and image.shape[1] > minSize[1]:
        image = rescale(image, scale ** i)
        images.append((scale ** i, image))
        i += 1
    # img = image.copy()
...
def pyramid_score(image,base_score, shape, stepSize=20, scale = 0.9, pixel_per_cell = 8):
    ...
    for i in range(len(images)):
        max_score_temp, maxr_temp, maxc_temp, response_map_temp = sliding_window(images[i][1], base_score, stepSize=stepSize, windowSize=shape, pixel_per_cell=pixel_per_cell)
        if max_score_temp > max_score:
            max_score = max_score_temp
            maxr = maxr_temp
            maxc = maxc_temp
            max_scale = images[i][0]
            max_response_map = response_map_temp
    ### END YOUR CODE
