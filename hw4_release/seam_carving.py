...
def energy_function(image):
    ...
    ### YOUR CODE HERE
    img = 0.30 * image[:, :, 0] + 0.59 * image[:, :, 1] + 0.11 * image[:, :, 2]
    Ix, Iy = np.gradient(img)
    out = np.abs(Ix) + np.abs(Iy)
    ### END YOUR CODE
...
def compute_cost(image, energy, axis=1):
    ...
    for h in range(1, H):
        for w in range(W):
            if w == 0:
                c_min = np.min([cost[h - 1, w], cost[h - 1, w + 1]])
                cost[h, w] = energy[h, w] + c_min
                if cost[h-1, w] == c_min:
                    paths[h, w] = 0
                else:
                    paths[h, w] = 1
                ...
                c_min = np.min([cost[h - 1, w], cost[h - 1, w - 1]])
                cost[h, w] = energy[h, w] + c_min
                if cost[h-1, w-1] == c_min:
                    paths[h, w] = -1
                else:
                    paths[h, w] = 0
            else:
                c_min = np.min([cost[h - 1, w], cost[h - 1, w + 1], cost[h - 1, w - 1]])
                cost[h, w] = energy[h, w] + c_min
                if cost[h - 1, w - 1] == c_min:
                    paths[h, w] = -1
                elif cost[h - 1, w] == c_min:
                    paths[h, w] = 0
                else:
                    paths[h, w] = 1
    ### END YOUR CODE
...
def reduce_fast(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    ### YOUR CODE HERE
    # Actually, it is not fast at all
    energy = efunc(out)
    while out.shape[1] > size:
        cost, paths = cfunc(out, energy)
        end = np.argmin(cost[-1])
        ...
        if i <= 3:
            ...
            energy = np.c_[energy[:, 0: i - 1], efunc(out[:, i - 3: j + 2])[:, 2: -1], energy[:, j + 2:]]    ### END YOUR CODE
