...
def harris_corners(img, window_size=3, k=0.04):
    ...
    square_dy = dy ** 2
    dx_dy = dx * dy
    # M = np.vstack((np.hstack((square_dx, dx_dy)), np.hstack((dx_dy, square_dy))))
    # M =  ndimage.convolve(M, window)
    # M_H, M_W = M.shape
    # print(M.shape)
    # print(img.shape)
    # for i in H:
    #     for j in W:
    #             response = np.linalg.det(M[i:i+3, j:j+3]) - k * (np.trace(M[i:i+3, j:j+3]) ** 2)
